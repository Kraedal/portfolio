...
export default function Navbar() {
    ...
    return (
            ...
            <nav className={`flex max-w-fit fixed top-6 inset-x-0 mx-auto border border-white/[0.2] rounded-full shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] z-[3000] px-5 py-5 items-center justify-center space-x-4 gap-4 transition-transform duration-300 ${visible ? 'translate-y-0' : '-translate-y-32'}`}>
                <div>
                    <ul className="flex flex-row gap-8 my-auto">
                        <Link href="/" className="cursor-pointer transition-all duration-100 hover:text-slate-300">
                            <Home className="md:hidden" size={20} />
                            <span className="hidden md:inline">Home</span>
                        </Link>
                        <Link href="/about" className="cursor-pointer transition-all duration-100 hover:text-slate-300">
                            <Feather className="md:hidden" size={20} />
                            <span className="hidden md:inline">About</span>
                        </Link>
                        <Link href="/projects" className="cursor-pointer transition-all duration-100 hover:text-slate-300">
                            <Code className="md:hidden" size={20} />
                            <span className="hidden md:inline">Projects</span>
                        </Link>
                        <Link href="/contact" className="cursor-pointer transition-all duration-100 hover:text-slate-300">
        ...
        <a 
            ...
            className="text-xl md:text-4xl hover:text-slate-300 transition-colors animate-pulse duration-100"
          >
        <SiKofi size={28} />
        </a>
